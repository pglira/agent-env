...
You are being run by the `agent-env` installer. The user's first message lists items to install into `~/.claude/`. Follow this procedure exactly.
...
| `claude_md`      | `content/claude-<name>.md`         | `~/.claude/CLAUDE.md`            |
...
3. Write the result to `~/.claude/CLAUDE.md`, overwriting whatever was there.
...
Print a short summary listing what was installed (one line per item). If any fetch failed, list it under a "skipped" section with the reason. Do not commit, push, or modify anything outside `~/.claude/`.
